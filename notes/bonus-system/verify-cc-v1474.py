#!/usr/bin/env python3
"""CC套（宫廷套装）各职业加成数值 - 稳态核查 v1474"""
import json, datetime

# CC套6件属性（70级全散件6件套）
STR, PA, IA, CR = 310, 110, 120, 3.0

# 校准值（经v1473及之前1400+轮验证确立）
...
chk("独立攻击收益(固伤)", 8.00, round(120/1500*100, 2))
chk("力量收益(百分比)", 77.5, 310/400*100)
...
result = {
    ...
    "core_data_drift": "核心数据零漂移",
    "results": checks
}

with open("verification-cc-bonus-v1474.json", "w") as f:
    json.dump(result, f, ensure_ascii=False, indent=2)

print(f"v1474 稳态核查: {passed}/{total} 通过 ({result['pass_rate']}%)")
for c in checks:
