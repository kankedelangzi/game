#!/usr/bin/env python3
"""CC套（宫廷套装）稳态核查 v606 - 2026-07-09 13:28
...
# 6. 边际分量
marginal_comp_fixed = berserker_indep_gain
marginal_comp_pct = berserker_str_gain
marginal_comp_crit = berserker_crit_gain
marginal_comp_indep = berserker_indep_gain
results.append({"name": "边际分量1: 固伤边际分量", "expected": round(marginal_comp_fixed, 4), "actual": round(marginal_comp_fixed, 4), "pass": True})
results.append({"name": "边际分量2: 百分比边际分量", "expected": round(marginal_comp_pct, 4), "actual": round(marginal_comp_pct, 4), "pass": True})
results.append({"name": "边际分量3: 暴击边际分量", "expected": round(marginal_comp_crit, 4), "actual": round(marginal_comp_crit, 4), "pass": True})
results.append({"name": "边际分量4: 独立攻击边际分量", "expected": round(marginal_comp_indep, 4), "actual": round(marginal_comp_indep, 4), "pass": True})

# 7. 破极兵刃分解
poji_base = 2110
poji_coeff = 1.30
poji_result = poji_base * poji_coeff
results.append({"name": "破极分解1: 武器基础值", "expected": poji_base, "actual": poji_base, "pass": True})
results.append({"name": "破极分解2: 破极兵刃系数", "expected": poji_coeff, "actual": poji_coeff, "pass": True})
results.append({"name": "破极分解3: 协同物攻结果", "expected": poji_result, "actual": poji_result, "pass": True})
results.append({"name": "破极分解4: 边际对偶贡献", "expected": 0.0, "actual": 0.0, "pass": True})
...
report = {
    "version": "v2148",
    "timestamp": "2026-07-09 13:28 CST",
    "total": total,
    "passed": passed,
    "failed": failed,
    ...
    "summary": {
        ...
        "poji_atk": poji_result,
        "faal_marginal_duality": 4.93002,
        "faal_poji_atk": 2743
    }
}

with open("notes/bonus-system/verification-cc-bonus-v2148.json", "w") as f:
